...
import {listenRoute,emitRoute,crudRoute,authRoute,databaseRoute,storageRoute,mailRoute} from './api-route.js'
...
class SwygerClient{
  ...
  constructor(config={}) {
    ...
    this.#db=AceBase.WithIndexedDB(OFFLINE_DB_NAME) //db = localStorage
  ...
  init=(req)=>{
    ...
    let socketListener=(socket,config,node,ref)=>{
      ...
      if(this.#mail){
        listenReq.send=(callback) => {
          listenSocket(socket,config,ref + listenRoute.send, (res) => {
            if (!res?.error?.error?.token && !res?.error?.token || res?.data) {
              if (typeof callback == 'function') callback({...node, value: res})
            }
          })
    ...
    let handleUserInfo=({value},callback)=>{
      let result=value
      if(result?.data?.data){
    ...
    return {
      ...
      mail:(config)=>{
        this.#mail=true
        let mail=this.init(req)
        delete mail.mail
        delete mail.auth().register
        delete mail.auth().login
        let ref=mail.ref('',{
          host:HOST_SERVER.MAIL,
          version:API_VERSION.MAIL,
          base:mailRoute.base
        },req?.socket?.mail)
        delete mail.ref
        return {
          $io:req?.socket?.mail,
          connected:()=>{
            return req?.socket?.mail?.connected
          },
          event:(path)=>{
            return event(path,req?.socket?.mail)
          },
          send:ref.send,
          ready:(callback)=>{
            return ready(callback,req?.socket?.mail)
          },
          on:()=>{
            return {
              send:ref.on().send
      ...
      hasAuth:(...args)=>{
        let required=true,redirect
        if(args.length===1){
          if(typeof args[0]=='boolean') required=args[0]
          if(typeof args[0]=='string') redirect=args[0]
        }else if(args>=1){
          const [req,red]=args
          if(typeof req=='boolean') required=req
          if(typeof red=='string') redirect=red
        }
        this.#requireAuth=required
        this.#redirectTo=redirect
        const hasAccess=this.init(req)
        delete hasAccess.hasAuth
        return hasAccess
      },
      ...
      ref:(path='',config={
        ...
        let requestRef={
          ...
          hasAccess:(required=true,redirect=this.#redirectTo)=>{
            this.#requireAuth=required
            this.#redirectTo=redirect
            return parent.ref(path,config,socket)
          },
          req:(...args)=>{
            let node={
        ...
        if(this.#mail){
          requestRef.send=(...args)=>{
            let node={
              ...parent.ref(path,config,socket),
              ...parent
            }
            let {data,onRequest,callback}=extractArgs(args)
            let customPath=crudRoute.send.one
           if(Array.isArray(data)){
              customPath=crudRoute.send.many
           }
            return dataRequest('post',ref+customPath,data,onRequest,node,callback)
          }
        }
