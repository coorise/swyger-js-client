let Config={
    init:(ClientConfig)=>{
        return {
            //Configure the offline DB
            OFFLINE_DB_NAME:ClientConfig?.OFFLINE_DB_NAME||'swyger_database',

            //Configure the server
            ENV:ClientConfig?.ENV||'prod',
            HOST_SERVER:{
                AUTH:ClientConfig?.HOST_SERVER?.AUTH||'http://localhost:4100',
                DATABASE:ClientConfig?.HOST_SERVER?.DATABASE||'http://localhost:4400',
                STORAGE:ClientConfig?.HOST_SERVER?.STORAGE||'http://localhost:4500',
                MAIL:ClientConfig?.HOST_SERVER?.MAIL||'http://localhost:4200',
            },
            API_VERSION:{
                AUTH:ClientConfig?.API_VERSION?.AUTH||'/api/v1',
                DATABASE:ClientConfig?.API_VERSION?.DATABASE||'/api/v1',
                STORAGE:ClientConfig?.API_VERSION?.STORAGE||'/api/v1',
                MAIL:ClientConfig?.API_VERSION?.MAIL||'/api/v1',
            },
            AUTO_REFRESH_TOKEN_TIMEOUT:ClientConfig?.AUTO_REFRESH_TOKEN_TIMEOUT||1500000, //in millisecond= 25 minutes
            API_KEY:ClientConfig?.API_KEY||'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..UvlcjBahR9G8dKxIeENJ7k4I87t3_oNJQ16eyPsRcSE'
        }
    }
}
export default Config

